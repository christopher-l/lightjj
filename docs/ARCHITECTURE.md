...
**3-pane merge editor** (`MergePanel.svelte`) — Per-file conflict editor. Raw marker content from `api.fileShow(rev, path, true)` (the `?snapshot=1` param forces `ui.conflict-marker-style=snapshot` so base is byte-exact in `-------` sections, no `%%%%%%%` diff reconstruction; opt-in only for the merge view — hunk-review must match user's style) goes through `conflict-extract.ts:reconstructSides()` which parses jj-native markers (`Diff`/`Snapshot` styles; `Git` style + 3+ sides fall through to raw FileEditor) into `{base, ours, theirs, blocks}`. The `blocks` array — one `ChangeBlock` per `<<<<<<<`/`>>>>>>>` region — is recorded at parse time; no LCS needed since the marker state machine already knows the boundaries. Three CM6 `EditorView`s (ours | result | theirs): flanks read-only, center seeded with `theirs` and editable. Arrow buttons copy flank content into the center at the tracked block position (`planTake` / `planTakeBoth` in `merge-surgery.ts`). A `blockTracker` StateField holds each block's center position as a LINE range (1-based half-open — never char offsets, which alias before/on/after a blank line); a take carries the target's new range plus a line `delta` shifting every later block (no `mapPos`), "All ours/theirs" is ONE composed transaction (one Cmd+Z), and hand-edits remap through `remapBlock`'s line-survival model (whole lines, never annexes an untouched shared neighbour) + `normalizeBlocks` (in-bounds/ordered/disjoint). `planTake` picks the `\n` separator by line arithmetic (source K=0 = delete + one separator — not `!insert`, blank lines are valid content). Every doc-changing transaction records a whole-tracker `restoreAll` snapshot via `invertedEffects`, so Cmd+Z/redo restore text AND every block's range/source exactly (joined history groups included — oldest snapshot wins). `[`/`]` block nav + "N of M" pill + minimap. Cmd+S → `merge-controller.save()`: `@` via `POST /api/file-write` (SSH-compatible), non-@ via `POST /api/merge-resolve` (`jj resolve --tool cp`, local-only — 501 in SSH).
